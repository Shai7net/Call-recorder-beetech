(() => {
  ...
  const frontViewPosition = () => isNarrowScreen() ? new THREE.Vector3(0, 30, 42) : new THREE.Vector3(0, 17, 24);
  const backViewPosition = () => isNarrowScreen() ? new THREE.Vector3(0, -34, -40) : new THREE.Vector3(0, -21, -24);
  ...
  const controllerTargetPosition = new THREE.Vector3(0, .25, 0);
  const controllerTargetScale = new THREE.Vector3(.92, .92, .92);
  const packageStartPosition = new THREE.Vector3(0, .1, 0);
  const packageStartScale = new THREE.Vector3(.78, .78, .78);
  const packageRestPosition = new THREE.Vector3(7.5, -2.1, -8);
  const packageRestScale = new THREE.Vector3(.44, .44, .44);
  const rgbObjects = [];
  ...
  function unboxPackage() {
    ...
    controllerRoot.scale.setScalar(.08);
    controllerRoot.position.set(0, -3.8, -2);
  ...
  function updateUnboxing(delta) {
    ...
    packageRoot.rotation.x = .12 * eased;
    controllerRoot.position.lerpVectors(new THREE.Vector3(0, -3.8, -2), controllerTargetPosition, eased);
    controllerRoot.scale.lerpVectors(new THREE.Vector3(.08, .08, .08), controllerTargetScale, eased);
    if (t >= 1) {
      state.unboxing = null;
      selectControl(
  ...
  function updateAnimations(delta) {
    for (let i = state.activeAnimations.length - 1; i >= 0; i--) {
      ...
      if (animation.camera) {
        ...
        controls.target.lerp(new THREE.Vector3(0, 0, 0), .14);
      } else {
  ...
  function updateHotspots() {
    ...
    const viewingFront = camera.position.y >= 0;
  ...
  canvas.addEventListener("pointerup", (event) => {
    ...
    if (!state.packageOpened && raycastPackage(event.clientX, event.clientY)) {
  ...
  canvas.addEventListener("dblclick", (event) => {
    if (raycastPackage(event.clientX, event.clientY)) unboxPackage();
  });
